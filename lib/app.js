...
module.exports.initialize = function (config) {
  ...
  // configure a session store based on postgres
  var pgSession = require('connect-pg-simple')(session);

  sessionstore = new pgSession({
    pg : pg,        // Use global pg-module
    conString : config.get("application").sessionStoreConnectString // Connect using something else than default DATABASE_URL env variable
  });
  app.use(session({ name: config.get("application").name_sid,
                    store:sessionstore,
  ...
  app.use(function (err, req, res, next) {
    ...
    console.log(err)
    res.render('500.pug', {
      message: 'Sorry, something went wrong and I cannot recover.',
      error: err
    })
  })
